...
const TIPS = [
  ...
  {
    emoji: '⚖️',
    title: b('Weigh in weekly, not daily', 'প্রতিদিন নয়, সপ্তাহে একবার ওজন মাপুন'),
    body: b(
      ...
      {TIPS.map((tip, i) => (
        <div key={i} className="tip-card fade-in">
          <div className="tip-header">
            ...
            <div className="tip-title">{lang === 'en' ? tip.title.en : tip.title.bn}</div>
          </div>
          <div className="tip-body">{lang === 'en' ? tip.body.en : tip.body.bn}</div>
        </div>
      ))}
    </div>
  );
